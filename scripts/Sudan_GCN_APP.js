// ═══════════════════════════════════════════════════════════════
//  SUDAN IMPROVED HYDROLOGIC CURVE NUMBER — v4.2 (GEE APP)
...
mainPanel.add(ui.Label('GCN250 Improved v4.2  |  GEE App  |  WorldCover 10m  |  SoilGrids HSG', {
...
mainPanel.add(makeButton('Clear Drawings', function() {
  ...
  var newLayer = ui.Map.GeometryLayer({geometries: null, name: 'geometry', color: '23cba7'});
  Map.drawingTools().layers().add(newLayer);
...
//  EXPORT
// ════════════════════════════════
mainPanel.add(sectionHeader('💾  DOWNLOAD / EXPORT'));

mainPanel.add(infoLabel('Select product:'));
var exportSelect = ui.Select({
  items: ['CN Average','CN Dry','CN Wet','CN Slope-Adjusted','Flood Risk'],
  value: 'CN Average',
  style: {stretch: 'horizontal', margin: '2px 0'}
});
mainPanel.add(exportSelect);

mainPanel.add(infoLabel('Resolution (meters):'));
var scaleSelect = ui.Select({
  items: ['250','500','1000','5000'],
  value: '1000',
  style: {stretch: 'horizontal', margin: '2px 0'}
});
mainPanel.add(scaleSelect);

// Download link label
var downloadUrlLabel = ui.Label('Click "Generate Download Link" above', {
  fontSize: '11px', color: '#0066CC', margin: '6px 0',
  whiteSpace: 'pre-wrap'
});

// Product helper
function getExportImage(product) {
  var imgMap = {
    'CN Average': cnAverage,
    'CN Dry': cnDry,
    'CN Wet': cnWet,
    'CN Slope-Adjusted': cnSlopeAdj,
    'Flood Risk': floodRisk
  };
  return imgMap[product];
}

// Method 1: Download link (works in App and Code Editor)
mainPanel.add(makeButton('Generate Download Link', function() {
  var product = exportSelect.getValue();
  var scale = parseInt(scaleSelect.getValue());
  var region = activeAOI;
  var img = getExportImage(product).clip(region).toFloat();
  ...
  resultLabel.setValue(
    ...
    'AOI: ' + activeAOIname + '\n\n' +
    'Please wait...');
  downloadUrlLabel.setValue('Generating...');
  
  img.getDownloadURL({
    name: product.replace(/ /g, '_'),
    scale: scale,
    region: region,
    crs: 'EPSG:4326',
    format: 'GEO_TIFF'
    ...
    if (error) {
      resultLabel.setValue(
        ...
        'TIPS:\n' +
        '1. Select a STATE or LOCALITY first\n' +
        '   (full Sudan is too large)\n' +
        '2. Use 1000m or 5000m scale\n' +
        '3. Draw a smaller region');
      downloadUrlLabel.setValue('Error - try smaller area or larger scale');
      return;
    }
    downloadUrlLabel.setValue('CLICK HERE TO DOWNLOAD');
    ...
    resultLabel.setValue(
      ...
      'Format: GeoTIFF\n' +
      '---\n' +
      'Click the blue "CLICK HERE TO\n' +
      'DOWNLOAD" link above.');
  });
}));

mainPanel.add(downloadUrlLabel);

// Method 2: Export to Google Drive (only works in Code Editor, not in App)
mainPanel.add(makeButton('Export to Google Drive (Code Editor only)', function() {
  var product = exportSelect.getValue();
  var scale = parseInt(scaleSelect.getValue());
  var region = activeAOI;
  ...
  var img = getExportImage(product).clip(region).toFloat();
  var taskName = product.replace(/ /g, '_') + '_' + suffix;
  ...
  resultLabel.setValue(
    ...
    'File saves to Drive > Sudan_CN\n\n' +
    'NOTE: This only works in Code\n' +
    'Editor, not in published App.');
}));

mainPanel.add(makeButton('Export ALL to Drive (5 products)', function() {
  var scale = parseInt(scaleSelect.getValue());
  var region = activeAOI;
  var suffix = activeAOIname.replace(/[^a-zA-Z0-9]/g, '_');
  var items = [
    [cnAverage,  'CN_Average_' + suffix],
    [cnDry,      'CN_Dry_' + suffix],
    [cnWet,      'CN_Wet_' + suffix],
    [cnSlopeAdj, 'CN_SlopeAdj_' + suffix],
    [floodRisk,  'Flood_Risk_' + suffix]
  ];
  items.forEach(function(e) {
    Export.image.toDrive({
      image: e[0].clip(region).toFloat(),
      description: e[1],
      folder: 'Sudan_CN',
      region: region,
      scale: scale,
...
var drawLayer = ui.Map.GeometryLayer({geometries: null, name: 'geometry', color: '23cba7'});
drawTools.layers().add(drawLayer);
...
// Welcome message (replaces print() which doesn't work in Apps)
resultLabel.setValue(
  '=== WELCOME ===\n' +
  'Sudan Hydrologic CN App v4.2\n' +
  '---\n' +
  'Click map: query CN + admin info\n' +
  'AOI: State / Locality / Draw / Asset\n' +
  '12 layers + auto legend\n' +
  'Runoff + GPM + Download GeoTIFF\n' +
  '---\n' +
  'Start by clicking anywhere on the\n' +
  'map or selecting an AOI above.');
